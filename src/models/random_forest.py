...
rmspe_rf = rmspe(y_valid, pred_rf)
mae_rf = mae(y_valid, pred_rf)
...
for name, val in imp_rf.head(15).items():
    print(f"   {name:<35} {val:.4f}")

plt.figure(figsize=(10, 6))
plt.barh(range(15), imp_rf.head(15).values[::-1])
plt.yticks(range(15), imp_rf.head(15).index[::-1])
plt.title('Random Forest — Топ-15 важных признаков', fontsize=14)
plt.tight_layout()
importance_path = FIGURES_DIR / 'importance_rf.png'
