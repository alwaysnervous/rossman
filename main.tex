...
% === МЕТАДАННЫЕ ===
\title{\textbf{Прикладные задачи анализа данных}\\[0.3em]\large Анализ вклада внешних и исторических признаков в качество прогнозирования спроса в розничной сети}
...
Объектом исследования являются дневные продажи магазинов сети Rossmann. Предметом исследования выступает вклад внутренних, исторических и внешних признаков в качество прогнозирования продаж.
...
\begin{itemize}
    ...
    \item сформировать базовый набор признаков, исторические признаки продаж и дополнительные внешние признаки;
    \item построить модели для различных наборов признаков;
    \item сравнить качество моделей с использованием метрик;
    \item определить группы признаков, вносящие наибольший вклад в качество прогнозирования.
\end{itemize}
...
Связь погодных условий с продажами также рассматривается в прикладных исследованиях розничной торговли. Например, Н.~Роуз и Л.~Долега анализируют связь между погодными условиями и ежедневными продажами крупного ритейлера, используя, в том числе, методы случайного леса \cite{rose2022weather}. Такие исследования показывают, что погодные признаки потенциально могут быть полезны для прогнозирования спроса, однако величина и направление эффекта зависят от товарной категории, региона и типа магазина.
...
\subsection{Общая схема исследования}

После анализа структуры исходных данных задача была сформулирована как \textbf{задача регрессии}: по информации о магазине, календарной дате, промо-акциях, исторических продажах и дополнительных внешних признаках необходимо предсказать дневной объём продаж \texttt{Sales}. Поскольку данные имеют временную природу, при построении и оценке моделей важно сохранять хронологический порядок наблюдений и не перемешивать обучающую и валидационную выборки случайным образом.
...
Погодные данные были получены через архивный API Open-Meteo \cite{openmeteo} для столиц федеральных земель Германии и агрегированы до дневного уровня за рабочие часы магазинов. Данные Google Trends \cite{googletrends} были сформированы для запроса \texttt{rossmann} по федеральным землям Германии. Показатель безработицы был получен из FRED \cite{fredunemployment} как месячный показатель \textit{Harmonized Unemployment Rate} для Германии. Внешние данные объединялись с основным датасетом по дате и региону; для месячного показателя безработицы значение месяца распространялось на все дни соответствующего месяца.
...
Отдельное внимание было уделено лаговым признакам продаж, поскольку они используют значения целевой переменной за предыдущие моменты времени. Для каждого магазина лаговые признаки вычислялись внутри соответствующего временного ряда после сортировки наблюдений по \texttt{Store} и \texttt{Date}. Поскольку строки с закрытыми магазинами были исключены, индекс \(t\) в формулах ниже обозначает номер доступного наблюдения для конкретного магазина, а не обязательно последовательный календарный день. При расчёте использовались только значения продаж, предшествующие текущему наблюдению:
\[
...
Признак \texttt{Open} указан как часть исходного набора данных, однако после исключения закрытых дней он принимает одно значение и поэтому не рассматривается как содержательно важный признак при интерпретации моделей.

Рассматриваемые дополнительные признаки можно разделить на две основные группы: исторические признаки продаж и внешние признаки. К историческим признакам относятся лаговые признаки, построенные по прошлым значениям продаж. К внешним признакам в данной работе относятся погодные данные, Google Trends, уровень безработицы и праздничные признаки, поскольку они формируются на основе источников, внешних по отношению к исходным таблицам \texttt{train.csv} и \texttt{store.csv}.
...
\subsubsection{Наивные базовые модели}
...
Для модели Random Forest важность признаков оценивалась на основе встроенной меры важности, рассчитываемой по уменьшению ошибки в деревьях ансамбля. Все важности в Random Forest нормируются так, что сумма важностей всех признаков равна 1. Наиболее важным для модели признаком оказался \texttt{Sales\_Lag1}, отражающий объём продаж в предыдущем доступном наблюдении для данного магазина. Его вклад существенно превышает вклад остальных признаков, что свидетельствует о сильной связи прогноза текущего наблюдения с уровнем продаж в предыдущем доступном наблюдении.
...
Для модели XGBoost был применён SHAP-анализ, позволяющий оценить вклад каждого признака в индивидуальные прогнозы модели. В отличие от обычной важности признаков, SHAP-значения позволяют рассматривать не только среднюю силу вклада признака, но и \textbf{направление этого вклада}. Согласно графику средней абсолютной важности, наибольший вклад в прогноз XGBoost вносят признаки \texttt{Sales\_Rolling7}, \texttt{Promo}, \texttt{Sales\_Lag1}, \texttt{DayOfWeek}, \texttt{day}, \texttt{IsFriday}, \texttt{WeekOfYear} и \texttt{Sales\_Lag7}. Таким образом, XGBoost, как и Random Forest, в первую очередь использует информацию о продажах за предыдущие доступные наблюдения, промо-активности и календарных закономерностях.
...
Summary-график SHAP дополнительно показывает характер вклада признаков в прогноз. Высокие значения \texttt{Sales\_Rolling7}, \texttt{Sales\_Lag1} и \texttt{Sales\_Lag7}, как правило, смещают прогноз продаж вверх, что подтверждает наличие инерционности продаж. Признак \texttt{Promo} также преимущественно даёт положительный вклад в прогноз, поскольку проведение промо-акций связано с ростом ожидаемых продаж. Для признака \texttt{DayOfWeek} наблюдается более сложная структура вклада: различные значения дня недели могут как увеличивать, так и уменьшать прогноз, что отражает недельную сезонность покупательского поведения.
...
\begin{itemize}
    ...
    \item В работе не моделировались возможные задержки публикации внешних данных. В прикладной системе прогнозирования необходимо дополнительно проверять, были ли соответствующие значения доступны на момент построения прогноза.
